...
The `.mat` release packs contain the generated artifacts required by the demo scripts, including certified executor data, trained controllers, baseline objects, trained network parameters, and evaluation data. The MATLAB live scripts load these release packs and reproduce the reported testing protocols, figures, timing summaries, feasibility checks, and closed-loop evaluation results.
...
|---|---|---|
| [`sim_region_compare.pdf`](sim_region_compare.pdf) | [`sim_region_compare.png`](sim_region_compare.png) | Geometry comparison between the explicit PWA optimizer partition and the certified active validity-region cover |
| [`sim_mpQP.pdf`](sim_mpQP.pdf) | [`sim_mpQP.png`](sim_mpQP.png) | Runtime, tail-latency, and hard-feasibility diagnostics for the mpQP benchmarks |
| [`sim_CA.pdf`](sim_CA.pdf) | [`sim_CA.png`](sim_CA.png) | Deadline-aware closed-loop control allocation trajectories and runtime effects |
| [`sim_ACC.pdf`](sim_ACC.pdf) | [`sim_ACC.png`](sim_ACC.png) | ACC closed-loop speed, control input, and safety-margin behavior |
...
This figure illustrates the difference between an objective-induced explicit optimizer partition and CertNet's feasibility-certified active cover. The explicit PWA solution partitions the parameter space according to the optimizer structure, which can lead to many critical regions. In contrast, CertNet focuses on the feasibility-induced structure: it deploys a smaller certified active cover to preserve hard feasibility, while leaving the performance-dependent selection to the learned policy inside this feasible family.
